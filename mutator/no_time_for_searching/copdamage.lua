...
module:hook(CopDamage, "die", function(self, variant)
	if Network:is_server() then
		...
		for _, enemy_data in pairs(managers.enemy:all_enemies()) do
			if
				enemy_data.unit:key() ~= key
				and radius > mvector3.distance_sq(pos, enemy_data.m_pos)
				and enemy_data.unit:character_damage():dodge(false)
			then
