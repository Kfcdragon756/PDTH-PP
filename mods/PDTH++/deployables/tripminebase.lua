...
module:hook(TripMineBase, "_explode", function(self, col_ray)
	...
	local damage_size = tweak_data.weapon.trip_mines.damage_size
		* managers.player:upgrade_value("trip_mine", "damage_multiplier", 1)
	local bodies =
		World:find_bodies("intersect", "cylinder", self._ray_from_pos, self._ray_to_pos, damage_size, slotmask)
			...
			if apply_dmg then
				...
				if hit_body:unit():id() ~= -1 then
					if player then
						...
						managers.network:session():send_to_peers_synched(
							"sync_body_damage_explosion_no_attacker",
							hit_body,
							normal,
							hit_body:position(),
							dir,
	...
	for _, data in pairs({
		{ count = 2, challenge = "dual_tripmine" },
		{ count = 3, challenge = "tris_tripmine" },
		{ count = 4, challenge = "quad_tripmine" },
	}) do
		if amount >= data.count then
			managers.challenges:count_up(data.challenge)
		end
	end

	if player then
		managers.network:session():send_to_peers_synched(
			"sync_trip_mine_explode",
			self._unit,
			player,
			self._ray_from_pos,
			self._ray_to_pos,
		...
		managers.network:session():send_to_peers_synched(
			"sync_trip_mine_explode_no_user",
			self._unit,
			self._ray_from_pos,
			self._ray_to_pos,
			damage_size,
